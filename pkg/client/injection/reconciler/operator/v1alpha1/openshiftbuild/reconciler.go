...
// controller reconciling v1alpha1.OpenShiftBuild.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.OpenShiftBuild. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.OpenShiftBuild) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.OpenShiftBuild.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.OpenShiftBuild. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.OpenShiftBuild) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.OpenShiftBuild if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.OpenShiftBuild.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.OpenShiftBuild) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.OpenShiftBuild) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.OpenShiftBuild resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operatorv1alpha1.OpenShiftBuildLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.OpenShiftBuild, desired *v1alpha1.OpenShiftBuild) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.OperatorV1alpha1().OpenShiftBuilds()
		...
		updater := r.Client.OperatorV1alpha1().OpenShiftBuilds()
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.OpenShiftBuild, desiredFinalizers sets.Set[string]) (*v1alpha1.OpenShiftBuild, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1alpha1.OpenShiftBuild, desiredFinalizers sets.Set[string]) (*v1alpha1.OpenShiftBuild, error) {
	...
	patcher := r.Client.OperatorV1alpha1().OpenShiftBuilds()
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1alpha1.OpenShiftBuild, desiredFinalizers sets.Set[string]) (*v1alpha1.OpenShiftBuild, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.OpenShiftBuild) (*v1alpha1.OpenShiftBuild, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.OpenShiftBuild, reconcileEvent reconciler.Event) (*v1alpha1.OpenShiftBuild, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
